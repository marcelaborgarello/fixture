...
export const PlayoffCard: React.FC<PlayoffCardProps> = ({ phase, config, type }) => {
  ...
  return (
    <CardInner config={config} className="flex flex-col justify-between h-full w-full">
      ...
      {/* Footer Branding */}
      <div className={`shrink-0 mt-auto flex items-end justify-center ${type === 'dieciseisavos' ? 'pt-0' : 'pt-1'}`} style={{ maxHeight: type === 'dieciseisavos' ? '30px' : '40px' }}>
        {config.showBrandingCards !== false && (
          <div className={`${type === 'dieciseisavos' ? 'scale-[0.6]' : 'scale-75'} origin-bottom`}>
            <BrandingPlaceholder
